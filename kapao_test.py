...
kapao_dir = osp.join(osp.dirname(osp.abspath(__file__)), "third_parties", "kapao")
os.chdir(kapao_dir)
sys.path.insert(0, kapao_dir)
import torch
import argparse
import yaml
...
import os.path as osp
import numpy as np
from ParallelCollaborativeInference._utils import iterate_all_close
import os
...
if __name__ == '__main__':
    import os.path as osp
    with open("data/coco-kp.yaml", "r") as f:
    ...
    model: torch.nn.Module = attempt_load("kapao_l_coco.pt", map_location=device)
    model.eval()
    ...
    dataset = LoadImages("res/crowdpose_100024.jpg", img_size=imgsz, stride=stride, auto=True)
    ...
    i = 0
    PCI = ParallelCollaborativeInference(offload=True, parallel_approach="select",ip="127.0.0.1", constraint_latency=False)
    PCI.start_client(model=model, init_forward_count=2)
    PCI.offload_order(30)
    # Init forward
    _ret = model(img, data, augment=False,
                 kp_flip=data['kp_flip'], scales=data['scales'], flips=data['flips'])
    for i in range(100):
        start = time.time()
        out = model(img, data, augment=False,
                    kp_flip=data['kp_flip'], scales=data['scales'], flips=data['flips'])
        end = time.time()
        i += 1
