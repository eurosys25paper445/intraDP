...
if __name__ == '__main__':
    ...
    test_set = dsets.CIFAR10(root='~/vgg19/dataset/CIFAR10/',
                             train=False,
                             download=True,
    ...
    vgg19.eval()
    import copy
    vgg19_local = copy.deepcopy(vgg19)
    ip = "192.168.10.13"
    port = 12345
    method = "select"
    PCI = ParallelCollaborativeInference(parallel_approach=method, ip=ip, port=port)
    PCI.start_client(model=vgg19, init_forward_count=1)
    test_iterator = iter(test_dl)


    x, y = next(test_iterator)
    pred_y = vgg19(x.to(device)).data.max(dim=1)[1]
    assert torch.allclose(pred_y, vgg19_local(x.to(device)).data.max(dim=1)[1])
    print("Finish init & profile forward\n\n")
