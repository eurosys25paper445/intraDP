    ...
    def generate_mixed_plan_2(self, bandwidth, partial_steps=3, linspace_num=6):
        ...
        def obj_func(x, no_numba=False):
            ...
            if len(transmit_indices) > 0:
                ...
                robot_comp_time[partial_mask] = np.clip(x[partial_mask], 0.5, 1.) * self.info.robot_ops_time[partial_mask]
                server_comp_time[partial_mask] = np.clip(x[partial_mask], 0.5, 1.) * self.info.server_ops_time[partial_mask]
                server_comp_time[(x>0) & (x < 1)] *= 1.2
