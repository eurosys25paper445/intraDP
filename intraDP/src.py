...
empty_preamble = b"0"*int(1024*1024*2) # 2M
class ParallelCollaborativeInference:
    ...
    async def start_common_loop(self, old_forward, input_queue: Queue, output_queue: Queue,
        ...
        if role == "client":
            ...
            await sock.send_obj(self.parallel_approach)
        else:
        ...
        async def profile_forward(warmup=2, repeat=3):
            ...
            if role == "client":
                sleep_step = max(int(5e-3 / profile_result.local_comp_time), 1)
            else:
                sleep_step = 20
            ...
            while True:
                if role == "client":
                    ...
                    sock.start_record_log(prefix + f"{count} th sock: ",
                                        ...
                                        max(plan["recv_num"], 1))   # recv_num / preamble
                else:
                    last_bandwidth = await sock.queued_recv()
                    plan = scheduler.graph_plan[last_bandwidth]
                    sock.start_record_log(prefix + f"{count} th sock: ",
                                        max(plan["send_num"], 1),   # send_num / preamble
